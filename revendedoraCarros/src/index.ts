...
ipcMain.handle('createUser',async (_: any, usuario: any) => {
  const {nome_usuario, data_nascimento, email_usuario, senha} = usuario
  const senhaHasheada = await hash(senha, 12)
  const novoUser = new Usuario(nome_usuario, data_nascimento, email_usuario, senhaHasheada);
  await new UserRepository().salvarUsuarioNoBanco(novoUser);
})


ipcMain.handle('procurar', async(_: any, email: string) =>{
  return await new UserRepository().verificarCadastroExistente(email);
})

ipcMain.handle('senhaHasheada',async (_: any, senha: any) => {
  const {senha_digitada, senhaHasheada} = senha
  return await compare(senha_digitada, senhaHasheada);
})